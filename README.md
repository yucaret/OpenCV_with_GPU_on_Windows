...
- Instalar el Git, lo pueden decargar de la siguiente ruta: https://git-scm.com/download/win en mi caso descargue la versión 2.33.1.
...
  
- Luego abrimos el prompt de Anaconda y creamos un ambiente (yo recomiendo instalar opencv en un ambiente nuevo debido a que las librerías ya instaladas en un ambiente existente puede causar conflictos en la configuración e instalación), llamaré mi ambiente con "pythonGPU" y lo voy a crear con la versión de python 3.8.5 utilizando el comando "conda create -n pythonGPU python=3.8.5":
...
  
- Configuramos la variable "OPENCV_EXTRA_MODULES_PATH" y allí colocamos la ruta del módulo del opencv_contrib (en mi caso es C:\opencv-gpu\opencv_contrib-4.x\modules):
...
- Se va a generar el siguiente error, en este caso para evitarlo hay que quitarle el check a la variable OPENCV_GENERATE_SETUPVARS.
